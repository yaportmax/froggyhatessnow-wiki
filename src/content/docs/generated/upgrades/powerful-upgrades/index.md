---
title: "Powerful upgrades"
description: "Powerful upgrades is a public upgrade concept from Steam copy or achievements."
draft: false
---

# Powerful upgrades

:::caution[Partially verified]
The listed sources verify this entry, but these fields still need confirmation: effect, unlock_method, mode.
:::

Powerful upgrades is a public upgrade concept from Steam copy or achievements.

| Field | Value |
|---|---|
| Status | Verified |
| Category | upgrades |
| Aliases | None listed |
| Effect | Needs verification. |
| Unlock Method | Needs verification. |
| Cost | Keys may be spent on treasure chests for powerful upgrades, per official Steam copy. |
| Mode | Needs verification. |
| Verified Fields | name, category, cost |
| Fields Needing Verification | effect, unlock_method, mode |
| Last Verified | Public Steam metadata accessed 2026-05-13 |
| Notes | Specific upgrade tree entries need verification. |

## Related

No related entities listed yet.

## Sources

- [Steam full-game store page](https://store.steampowered.com/app/3232380/FROGGY_HATES_SNOW/) — public_source, confidence high, source `steam-full-store`. Official public Steam store listing.
- [Steam demo store page](https://store.steampowered.com/app/4037600/FROGGY_HATES_SNOW_Demo/) — public_source, confidence high, source `steam-demo-store`. Official public Steam demo listing.
- [Steam community achievements page](https://steamcommunity.com/stats/3232380/achievements/?l=english) — public_source, confidence high, source `steam-full-achievements-page`. Public achievement names and descriptions.
