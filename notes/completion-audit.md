...
| Steam game sourcing prioritized | Snapshot generated `2026-05-13T15:39:50.593Z` includes all 15 Steam News API items from a 100-item request window, 11 direct Steam news/devlog sources, 70 confirmed Steam news/devlog terms, 42 achievement facts with public icon URLs, 20 parsed loadout-name rows, 14 full-game screenshots, 13 demo screenshots, Steam review summaries, external-source marker checks, public gameplay claims, and research gaps. | Done |
| Do not invent facts / mark uncertainty | Validator enforces allowed statuses; data includes 130 `Verified`, 27 `Inferred`, and 2 `Needs verification` rows. Roster/map/enemy/boss behavior gaps remain explicit. | Done |
| Structured datasets | 11 datasets exist with 159 entities: frogs 5, maps 5, tools 16, items 12, skills 29, companions 6, upgrades 7, bosses 4, enemies 4, achievements 42, glossary 29. | Done |
...
| Generated category/detail pages | `npm run generate` creates 159 entity detail pages and 11 category indexes; filesystem currently has 170 generated `index.md` files. | Done |
...
| Vercel deployment | Vercel project `yaportmax-5253s-projects/froggyhatessnow-wiki`; active deployment `dpl_8UYsVqC8BEkCweYPqWM6JEVQWd2p`; stable alias `https://froggyhatessnow-wiki.vercel.app` passes live checks including source marker `2026-05-13T15:39:50.593Z`. | Done |
...
- `npm run build`: 184 pages built; Pagefind indexed 183 pages.
