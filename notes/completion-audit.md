...
- `npm run deploy:status`: confirms the same two non-ready production deployments ahead of the latest successful ready deployment, verifies the live alias pages, and marks the non-ready deployment ids for manual Vercel review because Vercel reports aliases on them.
